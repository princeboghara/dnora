      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        {items.map((item, idx) => (
          ...
          >
            ...
            {/* Minimal Caption if present */}
            {item.title && (
              <div className="absolute bottom-0 inset-x-0 p-3 bg-gradient-to-t from-black/70 via-black/30 to-transparent text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      {activeImage && (
        ...
        >
          ...
          >
            ...
            {activeImage.title && (
              <p className="text-white text-xs sm:text-sm font-medium tracking-wider uppercase mt-4 text-center">
                {activeImage.title}
              </p>
            )}
          </div>
